...
if fidelity == "bun":
    ...
    bunsize = len(data["bundles"]["MF"][0]["HF"]) + len(
        data["bundles"]["MF"][0]["LF"]
    )  # assuming all bundles are the same size!
...
def scenario_creator(
    scenario_name, use_integer=False, sense=pyo.minimize, num_scens=None
):
    ...
    if "HF" == scenario_name[:2] or "LF" == scenario_name[:2]:
        ...
        model = pysp_instance_creation_callback(
            scenname, use_integer=use_integer, sense=sense
        )
        ...
        if num_scens is not None:
            if scen_fidelity == "HF":
                model._mpisppy_probability = data["scenarios"][scen_fidelity][
                    scenID_HFlist.index(scennum)
                ]["Probability"]
            elif scen_fidelity == "LF":
                model._mpisppy_probability = data["scenarios"][scen_fidelity][
                    scenID_LFlist.index(scennum)
                ]["Probability"]
            else:
        ...
        """
        assert (
            len(data["bundles"]["MF"][bundID_list.index(bundnum)]["LF"])
            + len(data["bundles"]["MF"][bundID_list.index(bundnum)]["HF"])
            == bunsize
        )

        # name each LF and HF scenario in the bundle
        snames = [
            f"HF_{data['bundles']['MF'][bundID_list.index(bundnum)]['HF'][i]}"
            for i in range(len(data["bundles"]["MF"][bundID_list.index(bundnum)]["HF"]))
        ]
        snames.extend(
            [
                f"LF_{data['bundles']['MF'][bundID_list.index(bundnum)]['LF'][j]}"
                for j in range(
                    len(data["bundles"]["MF"][bundID_list.index(bundnum)]["LF"])
                )
            ]
        ...
        bunkwargs = {"use_integer": use_integer, "sense": sense, "num_scens": None}
        ...
        bundle._mpisppy_probability = data["bundles"]["MF"][bundID_list.index(bundnum)][
            "Probability"
        ]
        return bundle
    else:
...
def pysp_instance_creation_callback(
    ...
    Demand = {}
    Demand["HF"] = {
        data["scenarios"]["HF"][i]["ID"]: data["scenarios"]["HF"][i]["Demand"]
        for i in range(len(data["scenarios"]["HF"]))
    }
    Demand["LF"] = {
        data["scenarios"]["LF"][j]["ID"]: data["scenarios"]["LF"][j]["Demand"]
        for j in range(len(data["scenarios"]["LF"]))
    }
    ...
    model.Demands = pyo.Param(
        within=pyo.NonNegativeReals, initialize=Demand_init, mutable=True
    )
    ...
    model.RegularTime = pyo.Var(
        model.Time, within=pyo.NonNegativeIntegers
    )  # number of units produced in reg. time at month t
    model.Storage = pyo.Var(
        model.Time, within=pyo.NonNegativeIntegers
    )  # number of units stored at month t
    model.Overtime = pyo.Var(
        model.Time, within=pyo.NonNegativeIntegers
    )  # number of units produced in OT at month t

    ## CONSTRAINTS

    def RTProduction_rule(
        model, t
    ):  # max. two units produced in reg. time for each month
...
def scenario_names_creator(num_scens, start=None):
    ...
    if bunsize == 0:
        ...
        if fidelity == "HF":
            scenvec = [
                f"HF_{data['scenarios']['HF'][i]['ID']}"
                for i in range(start, start + num_HFscens)
            ]
        elif fidelity == "LF":
            scenvec = [
                f"LF_{data['scenarios']['LF'][j]['ID']}"
                for j in range(start, start + num_LFscens)
            ]
        else:
            raise RuntimeError(f"No scenario fidelity specified")
        return scenvec
        ...
        return [
            f"bundle_{data['bundles']['MF'][i]['ID']}"
            for i in range(start, len(data["bundles"]["MF"]))
        ]
...
def bundle_hack(cfg):
    ...
    if cfg.bundle_size > 1:
        assert (
            cfg.num_scens % cfg.bundle_size == 0
