...
class ProgressiveHedgingSolver(object):
    ...
    def solve(self, sp, *, rho=None, max_iterations=None, solver=None, solver_options=None, loglevel=None):
        #
        # Misc configuration
        #
        if rho:
        ...
        while True:
            ...
            for b in sp.bundles:
                w[b] = {}
                for x in sfs_variables:
                    w[b][x] = w_prev[b][x] + self.rho * (sp.get_variable_value(b,x) - x_bar[x])
            ...
            if g < self.convergence_tolerance:
                logger.info(f"Termination: convergence tolerance ({g} < {self.convergence_tolerance})")
                break

            self.update_rho()
            k += 1
            if k == self.max_iterations:
                logger.info(f"Termination: max_iterations ({k} == {self.max_iterations})")
                break
