...
def bundle_by_fidelity(data, bundle_args=None):
    ...
    if any(data['scenarios'][i].get('Fidelity') == None for i in range(len(data['scenarios']))):
...
def single_scenario(data, bundle_args):
    ...
    for j in range(len(scens)):
        ...
        bundle[str(scens[j]['ID'])] = {'IDs':                    [scens[j]['ID']], 
                                       'Probability':            bun_prob, 
                                       'Scenario_Probabilities': {scens[j]['ID']: scens[j]['Probability']/bun_prob}}
...
def single_bundle(data, bundle_args):
    ...
    bundle['bundle'] = {'IDs':                    [scens[j]['ID'] for j in range(len(scens))], 
                        'Probability':            bun_prob, 
                        'Scenario_Probabilities': {scens[j]['ID']: scens[j]['Probability']/bun_prob for j in range(len(scens))}}
...
def bundle_random_partition(data, bundle_args):
    ...
    for idx, scen in enumerate(scens): # using indices rather than IDs to allow for non-numeric scenario IDs
        scen_idx.append(idx)
    ...
    for bun_idx in range(num_buns):
        bun_prob = sum(scens[temp_bundle[bun_idx][l]]['Probability'] for l in range(len(temp_bundle[bun_idx])))
        bundle[f'rand_{bun_idx}'] = {'IDs': [scens[temp_bundle[bun_idx][l]]['ID'] for l in range(len(temp_bundle[bun_idx]))], 
                                     'Probability': bun_prob,
                                     'Scenario_Probabilities': {scens[temp_bundle[bun_idx][l]]['ID']: scens[temp_bundle[bun_idx][l]]['Probability']/bun_prob for l in range(len(temp_bundle[bun_idx]))}}
...
def bundle_scheme(data, scheme_str, bundle_args=None):
    ...
    if abs(sum(bundle[key]['Probability'] for key in bundle.keys()) - 1.0) > 1e-04:
        raise RuntimeError (f"Bundle probabilities sum to {sum(bundle[key]['Probability'] for key in bundle.keys())}")
    ...
    for key in bundle.keys():
        if abs(sum(bundle[key]['Scenario_Probabilities'].values()) - 1.0) > 1e-04:
            raise RuntimeError (f"Scenario probabilities within bundle {key} do not sum to 1")
