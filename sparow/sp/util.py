            ...
            for v in block.component_objects(pyo.Var, active=True):
                if not isinstance(v, pyomo.core.base.var.IndexedVar):
                    ...
                    for _v in v.index_set():
                        if (v[_v].domain is pyo.Binary) and v[_v].name not in var_data_obj:
                            v[_v].domain = pyo.Reals
                            v[_v].bounds = (0,1)
                        elif (v[_v].domain is pyo.Integers) and v[_v].name not in var_data_obj:
                            v[_v].domain = pyo.Reals
                        elif (v[_v].domain is pyo.Boolean) and v[_v].name not in var_data_obj:
                            v[_v].domain = pyo.Reals
                            v[_v].bounds = (0,1)
                        elif (v[_v].domain is pyo.NonNegativeIntegers) and v[_v].name not in var_data_obj:
                            v[_v].domain = pyo.NonNegativeReals
                        elif (v[_v].domain is pyo.NegativeIntegers) and v[_v].name not in var_data_obj:
                            v[_v].domain = pyo.NegativeReals
                        elif (v[_v].domain is pyo.NonPositiveIntegers) and v[_v].name not in var_data_obj:
