...
class TestBendersTransforms:
    ...
    def test_transform_to_subproblem_model_domain_changes(self):
        ...
        for d in domains_to_check:
            ...
            assert m.s[None,1].x.domain is d, f"Expected first_stage_variable to be set to domain: {d}"
            assert all(m.s[None,1].y[i].domain is pyo.Reals for i in m.s[None,1].y.index_set()), f"Expected all second_stage_variables to stay as pyo.Reals"
    ...
    def Xtest_transform_to_subproblem_model_update_objective_1(self):
        ...
        for c in c_values:
            for r_option in removal_options:
                ...
                m.pprint()
                print(f"Obj transform update: {str(m.s[None,1].obj.expr)=}, {m.s[None,1].obj.active}")
                ...
                assert compare_expressions(split.not_in_set, app_data["R"] * m.s[None,1].y["Right"] + app_data["L"] * m.s[None,1].y["Left"])
    ...
    def test_transform_to_subproblem_model_update_objective_1(self):
        ...
        for c in c_values:
            for r_option in removal_options:
                ...
                split = split_expr(
                    m.obj.expr, [m.s[None,1].x], allow_iterables=True
                )
                assert split.constant == pytest.approx(0)
                assert compare_expressions(split.not_in_set, app_data["R"] * m.s[None,1].y["Right"] + app_data["L"] * m.s[None,1].y["Left"])
                if r_option:
